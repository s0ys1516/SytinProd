...
app.get('/sytin', (req, res) => {
	if (typeof user !== 'object') return res.sendFile(__dirname + '/sytin.html')
	res.send(`Регистрация прошла успешно! Данные учетной записи отправлены на email: ${user.email}`)
	user = undefined
})

app.listen(PORT, () => console.log(`server listening at http://localhost:${PORT}/sytin`))